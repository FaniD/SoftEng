...
import gov.nist.sip.proxy.newuser.NewUser;
import gov.nist.sip.proxy.online.Online;
...
import gov.nist.sip.proxy.forwarding.*;
...
/****************************** Messages ***********************************************************/
	...
	if ( request.getMethod().equals(Request.MESSAGE) ) {
		String msg = new String(request.getRawContent(), "UTF-8");
		String[] splited = msg.split("\\s+");
		if (splited[0].equals("Block")) {
			String blocker = splited[1];
			String blockee = splited[2];
			//Blocking
			Blocking bl = new Blocking(blockee, blocker);
			bl.newBlock();
		}
		else if (splited[0].equals("Unblock")) {
				String blocker = splited[1];
				String blockee = splited[2];
				//Unblocking
				Blocking bl = new Blocking(blockee, blocker);
				if (bl.check_if_blocked())
					bl.stopBlocking();			
		}
		else if (splited[0].equals("Forward")) {
				String from = splited[1];
				String to = splited[2];
				//Forwarding
				Forwarding frw = new Forwarding(from);
				//If forward already exists, we remove the old one and add new record!!!!
				if (frw.check_if_forwarding()!=null) {
					Response response=messageFactory.createResponse(Response.USE_PROXY,request);
             				if (serverTransaction!=null)
		...
		else if (splited[0].equals("Unforward")) {
			String from = splited[1];
			String to = splited[2];
			//Forwarding
			Forwarding frw = new Forwarding(from);
			if (frw.check_if_forwarding()!=null) {
				if (frw.check_if_forwarding().equals(to))
					frw.removeForward();
			}				
		}
		else if (splited[0].equals("Friend")) {
			String caller = splited[1];
			String callee = splited[2];
			//Enfriend
			Billing b = new Billing(caller, callee);
			b.setFriendCallee();
			if (b.get_friend_alert()) {
			//Send a message to user to inform him that he already has 5 friends
		...
		else if (splited[0].equals("Unfriend")) {
			String caller = splited[1];
			String callee = splited[2];
			//Enfriend
			Billing b = new Billing(caller, callee);
			if (b.checkFriendCall()) {
				b.removeFriendCallee();
			}			
		}
		else if (splited[0].equals("Business")) {
			String caller = splited[1];
			String callee = splited[2];
			//Business
			Billing b = new Billing(caller, callee);
			...
			if (b.get_biz_alert()) {
			//Send a message to user to inform him that he already has 5 friends
				Response response=messageFactory.createResponse(Response.MULTIPLE_CHOICES,request);
             			if (serverTransaction!=null)
                    			serverTransaction.sendResponse(response);
             			else sipProvider.sendResponse(response);
			}		
		}
		else if (splited[0].equals("Unbusiness")) {
			String caller = splited[1];
			String callee = splited[2];
			//Enfriend
			Billing b = new Billing(caller, callee);
			if (b.checkBusinessCall()) {
				b.removeBusinessCallee();
			}			
		}
		else if (splited[0].equals("Password")) {
		//	String login_signup = splited[1];
		//	int i = Integer.parseInt(login_signup);			
			String user = splited[1];
			String psw = splited[2];

			String name = splited[3];			
			String surname = splited[4];
			String email = splited[5];
			String address = splited[6];
			//Save to passwords.xml
		//	if (i==2) {
				NewUser nu = new NewUser(user);
				if (!nu.existingUser()) {
					nu.savePassword(psw);
...
/*		else if (splited[0].equals("UserInfo")) {
			
			String name = splited[3];			
			String surname = splited[4];
			String email = splited[5];
			String address = splited[6];
			String user = splited[5];
		...
		else if (splited[0].equals("Unregister")) {
			String name = splited[1];
			Online on = new Online(name);
			if (on.existingUser())
				on.removelogin();
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   EDO EINAI ETOIMI NA GINEI I KLISI KAI KOITAO BLOCKING KAI FORWARDING
   KATHORIZO CALLER CALLEE TELIKOUS (TYXON FORWARDING I BLOCKED) KAI FTIAXNO TO INSTANCE TOU BILLING 
   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
*/


/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	//A call is trying to start
   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
*/
	String caller = null;
	String callee = null;
	
	String call_method=request.getMethod();
	URI caller_URI = ( (FromHeader) request.getHeader(FromHeader.NAME)).getAddress().getURI();
	URI callee_URI = ( (ToHeader) request.getHeader(ToHeader.NAME)).getAddress().getURI();
	caller = ((SipURI)caller_URI).getUser();
	callee = ((SipURI)callee_URI).getUser();
	String id = ((CallIdHeader) request.getHeader(CallIdHeader.NAME)).getCallId();
			
	//First of all we check if the callee user is online
	//If not online, no block or forward take place
	Online onl = new Online(callee);
	if (!onl.existingUser()) {
		//User is not online, send the appropriate message back to caller
		//Callee offline
		//Use Proxy
    		Response response = messageFactory.createResponse(Response.USE_PROXY,request);
...
/************* Blocking & Forwarding ***********************************************************/
  ...
  if(request.getMethod().equals(Request.INVITE)) {
	  try {
    	Blocking bl = new Blocking(caller,callee);
    	Forwarding fw = new Forwarding(callee);
    	if (bl.check_if_blocked()) {
	
	//Callee currently unavailable
	//301 Moved Permanently
    		Response response = messageFactory.createResponse(Response.MOVED_PERMANENTLY,request);
    	...
    	String forward_user = fw.check_if_forwarding();
	...
	//The client should try at the address in the Contact field. If an Expires field is present, the client may cache the result for that period of time.
		if (!fw.add_callee_without_cycle(caller)) {
			 
			Response response = messageFactory.createResponse (Response.MULTIPLE_CHOICES, request);
			SipURI URIfix = (SipURI) addressFactory.createURI("sip:"+callee);
			ContactHeader contactHeader = headerFactory.createContactHeader (addressFactory.createAddress(URIfix));
...
/*********** Billing***********************************************************/
	...
	if (call_method.equals(Request.ACK)) {
		Billing b = new Billing(caller,callee);
		b.callStart();
        }
    if (call_method.equals(Request.BYE)) {
		Billing b = new Billing(caller,callee);
		b.callEnd();
		b.costCalculate();
		Forwarding fwd = new Forwarding(callee);
		fwd.delete_path_record(caller,callee,-1); //if it doesn't exist that's ok
    }
/********************************************************************************/    
